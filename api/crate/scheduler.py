...
dependency. Three loops, all on UTC clocks:
...
Passes skip quietly (one log line, re-armed after the next success) while no
usable Spotify credential exists, so a disconnected or reauth-pending account
never spams the log every tick.
"""
